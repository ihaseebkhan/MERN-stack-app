...
export default class Main extends Component {
  ...
  componentDidMount() {
    ...
    axios.get('http://localhost:3000/readMemorialsApproved')
      .then((res) => {
...
var seconds = Math.floor(delta % 60);  // in theory the modulus is not required
    ...
    <p class="card-text"><small class="text-muted" style={{ fontSize: "0.625em" }}>{time}</small><Link><FontAwesomeIcon className="text-muted float-right my-2" data-toggle="tooltip" data-placement="top" title="Share this memorial" data-toggle="modal" data-target="#shareModal" onClick={()=>this.setState({shareId:item.deceased_id})} icon={faShareAlt} /></Link><Link to={{pathname:`/tribute/${item.deceased_id}`}}><FontAwesomeIcon className="text-muted float-right my-2 mr-3" data-toggle="tooltip" data-placement="top" title="Post tribute" icon={faHeart} /></Link></p>
            </div>
          </div>
        </div>
      </div>)
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body text-center">
                ...
                <WhatsappShareButton className="mr-4" url={window.location.href+"tribute/"+shareId}><WhatsappIcon size={32} round={true} /></WhatsappShareButton>
